...
# Built-in Win32 class name → UIA role mappings (standard controls only).
# Application-specific mappings are registered at runtime via
# ``register_class_role()`` from skill plugins.
...
# Extended at runtime by ``register_interactive_class()`` from skill plugins.
_WIN32_INTERACTIVE_CLASSES: set[str] = {
...
def register_class_role(class_name: str, role: str) -> None:
    """Register a custom Win32 class-name → role mapping.

    Called by skill plugins to teach the bridge about app-specific controls.
    """
    _WIN32_CLASS_TO_ROLE[class_name.lower()] = role


def register_interactive_class(class_name: str) -> None:
    """Register a Win32 class-name as interactive.

    Called by skill plugins to mark app-specific controls as keyboard-navigable.
    """
    _WIN32_INTERACTIVE_CLASSES.add(class_name.lower())
