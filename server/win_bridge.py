...
class WinUIABridge(UIABridge):
    ...
    # ---------------------------------------------------------------------------
    # Register state reading (reconcile / register view helpers)
    # ---------------------------------------------------------------------------
    ...
    def read_register_state(self) -> dict[str, Any]:
        ...
        ------
        UIAError
            ``REGISTER_NOT_FOUND`` if no TxList is visible.
        """
        import ctypes  # noqa: PLC0415
        import ctypes.wintypes  # noqa: PLC0415
    ...
    def set_register_filter(self, text: str) -> dict[str, Any]:
        ...
        Works in both normal register view (the global search bar) and the
        reconcile register view (the inline filter Edit).

        Parameters
        ----------
        text : str
            Search term — e.g. ``"AMAZON"`` or ``"4/12/2026"``.
        ...
        UIAError
            ``REGISTER_NOT_FOUND`` if no TxList is visible.
            ``FILTER_EDIT_NOT_FOUND`` if no filter edit is found.
        """
        import ctypes  # noqa: PLC0415
        import ctypes.wintypes  # noqa: PLC0415
        ...
        time.sleep(0.4)  # allow Quicken to re-render
